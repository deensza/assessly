  ...
  return (
    <header className="sticky top-0 h-14 bg-white border-b border-gray-200/80 flex items-center justify-between px-6 z-[100]">
      ...
      <div className="flex items-center gap-5">
        <Link
          href={user ? (user.role === "admin" ? "/admin" : user.role === "instructor" ? "/instructor" : "/student") : "/"}
      ...
      <div className="flex items-center gap-1">
        ...
        {isAuthenticated && user ? (
          ...
          <Link href="/login" className="text-sm font-semibold bg-[#4a90e2] text-white px-4 py-2 rounded-lg hover:bg-[#357abd] transition-all">
            Sign In
          </Link>
        )}
      </div>
